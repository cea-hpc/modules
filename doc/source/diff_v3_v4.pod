...
When locating I<modulefiles> on Modules 4, if a F<.modulerc>, a F<.version>, a directory or a I<modulefile> cannot be read during the search it is simply ignored with no error message produced. Visibility of I<modulefiles> can thus be adapted to the rights the user has been granted. Exception is made when trying to directly access a directory or a I<modulefile>. In this case, the access issue is returned as an error message. Access issue is also returned when a direct access is made to a module alias or a symbolic version targeting an unreadable I<modulefile>.
