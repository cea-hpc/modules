...
where I<E<lt>shellE<gt>> is the name of the sourcing shell. For example, a C
Shell user sources the F<$MODULESHOME/init/csh> script. The sh, csh, tcsh,
bash, ksh, and zsh shells are supported by B<modulecmd.tcl>. In addition,
python and perl "shells" are supported which writes the environment changes
to stdout as python or perl code.
...
Switch loaded I<modulefile1> with I<modulefile2>
