...
Returns the integer value of B<modulecmd.tcl>'s flags state.
...
error messages being produced and on the behavior of B<modulecmd.tcl> in case of
detecting an outage.
...
Switches verbose B<modulecmd.tcl> message display on or off.
...
one of the following values:
	...
	warn - warnings
	prob - problems (normally the B<modulecmd.tcl> may be completed)
	error - errors (which normally leads to unsuccessful  end
		of the B<modulecmd.tcl>)
...
following values are allowed:
	...
	output to  this  stream  might  screw  up  the  B<modulecmd.tcl>
